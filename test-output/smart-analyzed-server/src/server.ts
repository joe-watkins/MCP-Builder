...
export class SmartAnalyzedServerServer {
  private server: Server;

  constructor() {
    this.server = new Server(
      {
        name: 'smart-analyzed-server',
        version: '1.0.0',
      },
      {
        capabilities: {
  ...
  private setupErrorHandling(): void {
    this.server.onerror = (error) => {
      console.error('[smart-analyzed-server] Error:', error);
    };
  ...
  async start(): Promise<void> {
    ...
    console.error('[smart-analyzed-server] Server started successfully');
  }
}
